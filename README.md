...
1. **首次点击永不中雷**: 第一次揭开时**才**生成雷, 且避开点击位置周围 3×3 区域 (含自身)
2. **随机布雷**: 候选池 = 棋盘所有格子 − 3×3 安全区, 用 `Array.shuffle()` 打乱后取前 `mines` 个标记 `is_mine = true`
3. **数字 = 周围 8 格雷数**: 每个非雷格立即扫描 8 邻居, 雷数存到 `adjacent_mines`; 揭开时显示该数字 (0 = 空白, 自动连锁展开)
4. **胜利**: 揭开所有非雷格子自动判定, 剩余未揭格必然全是雷 → 自动插旗
5. **失败**: 左键点中雷 → 该雷红色高亮, 揭开其他未标雷, 错标的旗子画红 ✗
6. **边界**: 角落 3 邻居 / 边 5 邻居, `_count_adjacent_mines` 内部已做边界检查
7. **不存盘**: 每次新游戏重新生成, 同一难度每次雷图都不同 (理论最优解不固定)
